...
var ractive = new OneDecisionApp({
  ...
  data: {
    ...
    stdPartials: [
      ...
      { "name": "navbar", "url": "/partials/dmn-navbar.html"},
...
//      data._links = {self:{href:location}};
  ...
  delete: function (definition) {
    ...
    $.ajax({
        url: ractive.getSelfLink(definition),
        type: 'DELETE',
  ...
  getSelfLink: function(definition) {
    console.info('getSelfLink');
    for (idx in definition.links) {
      if (definition.links[idx].rel=='self') return definition.links[idx].href;
    }
    ractive.showError('No decision model selected!');
  },
  oninit: function() {
    console.info('oninit');
    this.ajaxSetup();
  ...
  select: function(definition) {
    ...
    var url = ractive.tenantUri(ractive.getSelfLink(definition)+'/');
