...
public class DecisionRule implements TestRule {
    ...
    public static String loadFromClassPath(String resource) {
        try (Scanner scanner = new Scanner(
                DecisionRule.class.getResourceAsStream(resource))) {
    ...
    public void writeDmn(Definitions decisionModel, String filename) throws IOException {
        File dmnFile = new File(outputDir, filename);
        System.out.println(String.format("Writing DMN to %1$s",
                dmnFile.getAbsolutePath()));
        
        try (Writer out = new FileWriter(dmnFile)) {
            getDecisionEngine().getRepositoryService()
                    .write(decisionModel, out);
        }
    }

    public File writeHtml(String html, String fileName) throws IOException {
    ...
    public File writeJson(Definitions definitions, String fileName)
            throws IOException {
        File file = new File(DecisionRule.outputDir, fileName);
        System.out.println(
                String.format("Writing JSON to %1$s", file.getAbsolutePath()));

        om.writeValue(file, definitions);
        return file;
    }
