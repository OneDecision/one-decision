...
public class DecisionModelFactory implements DecisionConstants,
    ...
    @Value("${onedecision.dmn.dmn2HtmlXslt:/static/xslt/dmn2html.xslt}")
    private String xsltResources = "/static/xslt/dmn2html.xslt";
    ...
    @Override
    public DmnModel createModelForTenant(String tenantId, InputStream is)
            throws IOException {
        Definitions model = load(is);
        return createModelForTenant(new DmnModel(model, tenantId));
    }
    ...
    protected TransformUtil getTransformUtil() {
        // if (transformUtil == null) {
            ...
            try {
                transformUtil.setXsltResources(xsltResources);
